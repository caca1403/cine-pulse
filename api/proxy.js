export default async function handler(req, res) {
  ...
  function cleanPath(sub) {
    return sub.split('/').map(seg => encodeURIComponent(decodeURIComponent(seg))).join('/');
  }
  ...
  if (fullPath.startsWith('hdfc')) {
    ...
    targetUrl = `https://www.hdfilmcehennemi.now/${cleanPath(sub)}${qs}`;
    ...
    targetUrl = `https://filmizle.now/${cleanPath(sub)}${qs}`;
    ...
    targetUrl = `https://vidmixi.com/${cleanPath(sub)}${qs}`;
    customHeaders['Referer'] = 'https://filmizle.now/';
  } else if (fullPath.startsWith('szd')) {
    const sub = fullPath.replace(/^szd\/?/, '');
    targetUrl = `https://sezonlukdizi.cc/${cleanPath(sub)}${qs}`;
