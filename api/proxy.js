export default async function handler(req, res) {
  ...
  function cleanPath(sub) {
    ...
    const parts = sub.split('/').filter(Boolean).map(seg => encodeURIComponent(decodeURIComponent(seg)));
    return parts.join('/') + (hasTrailing && parts.length > 0 ? '/' : '');
  }
  ...
  if (fullPath.startsWith('hdfc')) {
    ...
    targetUrl = `https://www.hdfilmcehennemi.now/${cleanPath(sub)}${userQs}`;
    ...
    targetUrl = `https://filmizle.now/${cleanPath(sub)}${userQs}`;
    ...
    targetUrl = `https://vidmixi.com/${cleanPath(sub)}${userQs}`;
    customHeaders['Referer'] = 'https://filmizle.now/';
