const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./PlayerModal-Ccb7h05n.js","./hls-BuERnqCp.js"])))=>i.map(i=>d[i]);
...
`).filter(Boolean).map(d=>`<li style="margin-bottom: 0.45rem;">${mo(d.replace(/^[•\-\*]\s*/,""))}</li>`).join("");n.innerHTML=`
  ...
  `,document.body.appendChild(n),G(n);const a=n.querySelector("#btn-update-later");a&&a.addEventListener("click",()=>{try{localStorage.setItem(jl,String(Date.now()+12*60*60*1e3))}catch{}n.remove()});const o=d=>{if(Z("APK indirmesi başlatılıyor...","info"),fi())try{window.open(d,"_system")}catch{window.location.href=d}setTimeout(()=>{try{n.remove()}catch{}},1800)},s=n.querySelector("#btn-update-download");s&&s.addEventListener("click",d=>{fi()&&d.preventDefault(),o(t)});const l=n.querySelector("#btn-update-github");l&&l.addEventListener("click",d=>{fi()&&(d.preventDefault(),o(i))})}function mo(e=""){return String(e).replace(/[&<>'"]/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"})[t])}async function Kl({manual:e=!1}={}){if(!e)try{const t=parseInt(localStorage.getItem(jl)||"0",10);if(Date.now()<t)return null}catch{}try{const t=await fetch(`${yu}?_t=${Date.now()}`,{signal:AbortSignal.timeout(6e3),cache:"no-store"});if(!t.ok)throw new Error(`HTTP ${t.status}`);const i=await t.json();if(i&&vu(i.version,fo))return bu(i),i;if(e)return Z(`✓ CinePulse güncel (v${fo})`,"success"),null}catch{return e&&Z("Güncelleme sunucusuna erişilemedi.","error"),null}}function wu(){typeof window>"u"||window.setTimeout(()=>{Kl({manual:!1}).catch(()=>{})},4e3)}let Xn=null;function ku(){qt();const e=document.createElement("div");e.id="profile-modal-root",e.className="profile-backdrop",document.body.appendChild(e),Xn=e;const t=(n="select")=>{const r=Jt(),a=xn();n==="select"?e.innerHTML=`
